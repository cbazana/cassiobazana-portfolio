...
(function () {
  ...
  var translations = {
    ...
    "Meu processo começa antes do Figma: entender o problema, o usuário e a métrica que precisa mudar. Uso pesquisa e dados para validar direção, desenho com consistência dentro de um design system e testo antes de declarar pronto. Colaboro de perto com produto e engenharia para que a solução saia do papel sem perder qualidade no caminho e uso IA como parte do processo, não como atalho, para acelerar exploração e validação sem abrir mão de critério.": "My process starts before Figma: understanding the problem, the user and the metric that needs to move. I use research and data to validate direction, design with consistency inside a design system, and test before calling anything done. I work closely with product and engineering so the solution ships without losing quality along the way, and I use AI as part of the process, not a shortcut, to speed up exploration and validation without giving up judgment.",
